"""Diagnostic: which columns get claimed as io_number per file?

For each file in extracted_2, list:
  - GT io_number column letter
...
def gt_io_col_for(file_name: str) -> str | None:
    ...
    for e in d["results"]:
        if e["file_name"] == file_name:
            plan = e.get("plan") or {}
            if plan.get("io_number"): return plan["io_number"]
    return None
...
def main():
    ...
    for name in files:
        ...
        # Group io_number findings by (label_coord, column)
        io_findings = [f for f in (tab + kv) if f.canonical == "io_number"]
        ...
        print(f"\n{name[:60]}")
        print(f"  GT io_number col: {gt_col or '(none)':<6}   predicted cols: {sorted(cols_claimed)}")
