"""Lookup tools — find_merged_cells_in_column + check_column_has_strip."""
...
def test_no_merges_yields_empty_set() -> None:
    ...
    assert find_merged_cells_in_column(canvas, col_idx=1, rows=[3, 4, 5]) == set()
...
def test_merge_in_target_column_returns_overlapping_cells() -> None:
    ...
    assert find_merged_cells_in_column(canvas, col_idx=1, rows=[3, 4, 5]) == {(3, 1), (4, 1)}
...
def test_horizontal_merge_spanning_target_column_returns_only_overlapping_rows() -> None:
    ...
    assert find_merged_cells_in_column(canvas, col_idx=1, rows=[3, 4]) == {(3, 1)}
...
def test_merge_outside_row_range_is_excluded() -> None:
    ...
    assert find_merged_cells_in_column(canvas, col_idx=1, rows=[3, 4]) == {(3, 1), (4, 1)}


def test_tool_registered_under_canonical_name() -> None:
    assert "find_merged_cells_in_column" in TOOL_REGISTRY.names()
...
def test_tool_registered_in_global_registry() -> None:
    assert "check_column_has_strip" in TOOL_REGISTRY.names()
